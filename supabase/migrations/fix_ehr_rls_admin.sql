...
--
-- Problem: Admin users (jerlessm@gmail.com, info@mindshiftwellnessclinic.org)
-- are not in clinician_roles, so the existing RLS policy blocks them.
-- Solution: Expand the policy to also allow users whose email is in the admin list.
...
CREATE POLICY "Clinicians and admins manage ehr_charts" ON ehr_charts
  ...
  USING (
    ...
    (SELECT email FROM auth.users WHERE id = auth.uid())
      IN ('info@mindshiftwellnessclinic.org','jerlessm@gmail.com',
          'kmutegyeki@mindshiftwellnessclinic.org','rnakkazi@mindshiftwellnessclinic.org')
  );
...
-- ── intake_submissions (clinician read/update) ────────────────────────────────
